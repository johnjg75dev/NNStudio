...
def is_admin(user: User) -> bool:
    """Check if user is admin (can add check for admin role in future)."""
    # For now, just ensure user is authenticated
    # TODO: Add admin role/permission system
...
def list_architectures():
    ...
    return jsonify({
        "architectures": [arch.to_dict() for arch in architectures]
    })
...
def update_architecture(key: str):
    ...
    return jsonify({
        "message": f"Architecture '{key}' updated",
